...
SaveCheckPoint.prototype.countRelevant = function (results) {
  var relevant = []

  relevant = results.filter(function (entry) {
    return entry.label === 1 || entry.label === -1
  })
...
SaveCheckPoint.prototype.loadLast = function () {
  ...
  // count relevant
  var relevant = this.countRelevant(results)
...
SaveCheckPoint.prototype.checkpoint = function () {
  ...
  // get the data
  var results = this.getData()
  var numberOfRelevantResults = this.countRelevant(results)
